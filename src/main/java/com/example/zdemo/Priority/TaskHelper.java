...
/**
 * TaskHelper TODO:此池暂未使用到 暂保留 20180621
 *
 * @date 2016/12/27
 */
public class TaskHelper {
    ...
    private static final int CORE_POOL_SIZE = 200;//线程池大小
    private static final int MAX_POOL_SIZE = 500;//线程池大小

    //创建线程池
    private static ThreadPoolExecutor orderTaskPool =
        new ThreadPoolExecutor(CORE_POOL_SIZE, MAX_POOL_SIZE,60,
            TimeUnit.SECONDS,new ArrayBlockingQueue<Runnable>(200));

    public static void executePushTask(Runnable task) {
        orderTaskPool.submit(task);
